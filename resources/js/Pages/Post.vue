<script setup></script>

<template>
    <div class="min-w-[450px]">
        <div class="flex justify-between w-[400px] items-center gap-3 mb-3">
            <div class="flex gap-3 mb-3">
                <div>
                    <img class="rounded-full" src="https://picsum.photos/seed/picsum/32/32" />
                </div>
                <div>
                    <p>Name</p>
                    <p>tag</p>
                </div>
            </div>
            <unicon name="ellipsis-h" fill="white"></unicon>
        </div>
        <img class="" src="https://picsum.photos/seed/picsum/400/650" />
        <div class="mt-3 mb-3 flex flex-row justify-between w-[400px]">
            <div class="flex gap-3">
                <unicon class="w-7 h-7" name="heart" fill="white"></unicon>
                <unicon class="w-7 h-7" name="comment" fill="white"></unicon>
                <unicon class="w-7 h-7" name="telegram-alt" fill="white"></unicon>
            </div>
            <div>
                <unicon class="w-7 h-7" name="bookmark" fill="white"></unicon>
            </div>
        </div>
        <div class="leading-8">
            <div>
                334,877 J'aime
            </div>
            <div>
                muftimenkofficial Morning Qur'aan Recitation
            </div>
        </div>
    </div>
</template>