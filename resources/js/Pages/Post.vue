...
const publishComment = (event) => {
  ...
  axios
    .post("/post/comment", { post_id: props.post.id, content: currentComment.value })
    .then((response) => {
      posts.addComment(response.data);
...
<template>
  <div
    v-if="isLoading"
    class="w-full max-w-screen sm:w-[570px] 2xl:w-[470px] h-[470px] mb-16"
  >
    <div class="flex gap-4 mb-4">
      ...
      <div>
        ...
        <div
          class="w-[130px] animate-pulse-bg mt-2 h-4 rounded-md bg-[hsl(0,0%,30%)]"
        ></div>
      </div>
    </div>
    ...
    <div>
      <div class="flex justify-between items-center gap-3 mb-3 px-3 sm:px-0">
        <div class="flex gap-3 mb-3">
          ...
          <div>
            <Link class="font-semibold" :href="'/profile/' + post.user.username">{{
              post.user.name
            }}</Link>
            <p>{{ post.location }}</p>
          </div>
      ...
      <div
        class="sm:max-h-[550px] overflow-hidden max-h-[50dvh] w-screen sm:w-[550px] xl:max-w-[470px] border border-[#262626] rounded flex items-center justify-center backdrop-blur-lg"
      >
        <div>
          <img
            v-if="post.image !== null"
            class="sm:max-h-[550px] object-contain max-h-[50dvh] w-full"
            :src="usePage().props.ziggy.url + post.image"
          />
          <div class="flex items-center justify-center hover:cursor-pointer" v-else>
            <video
              class="sm:max-h-[550px] relative object-contain max-h-[50dvh] w-full"
              ref="videoPlayer"
      ...
      <div class="leading-8 mb-3 px-3 sm:px-0">
        <div v-if="post.enable_likes">{{ post.numberOfLikes }} J'aime</div>
        ...
        <div v-if="post.enable_comments">
          <div class="hover:cursor-pointer text-[hsl(0,0%,45%)]" @click="getComments">
            Affichier les {{ post.numberOfComments }} commentaires
          </div>
          <div class="items-center hidden sm:flex justify-end gap-4 mt-1 h-[7%] relative">
            ...
            <button
              ...
              v-show="currentComment.length !== 0"
              :disabled="currentComment.length === 0"
            >
              Publier
            </button>
            <svg-icon
              class="hover:cursor-pointer min-w-[20px] float-right"
              type="mdi"
              size="14"
            ...
            <EmojiPicker
              ref="emojiRef"
              class="absolute bottom-[25%] right-[-285px] z-10"
