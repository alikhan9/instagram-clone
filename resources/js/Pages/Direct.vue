...
onMounted(() => {
    ...
    setTimeout(() => {
        if (
            props.receiver &&
            messages.getUnreadNotificationsForUser(props.receiver.id) > 0
        ) {
            axios
                .post('/message/notifications/check', {
                    sender: props.receiver.id,
                })
                .then(() => {
        ...
        if (props.receiver) {
            currentContact.value = props.contacts.filter(m =>
                m.receiver.hasOwnProperty('id')
                    ? m.receiver.id == props.receiver.id ||
                      m.receiver.id === user.id
                    : m.initiator.id == props.receiver.id ||
                      m.initiator.id === user.id
            )[0]
        }
    }, 10)
})
...
<template>
    <div
        class="fixed inset-0 left-0 top-0 z-[999] flex max-h-[100vh] w-full flex-col overflow-hidden bg-black text-white lg:relative lg:z-0 lg:h-full lg:flex-row"
    >
        <Menu
            :isMessageReady="isMessageReady"
            :receiver="receiver"
            v-model:currentContact="currentContact"
        ...
        >
            <div class="flex flex-col items-center gap-4">
                ...
                <div class="text-2xl">Vos messages</div>
                <div class="text-[hsl(0,0%,70%)]">
                    Envoyez des photos and des messages privés à un(e) ami(e) ou
                    à un groupe
                </div>
                <!-- TODO: Create new chat -->
                <button
                    class="rounded-lg bg-[rgb(0,149,246)] px-4 py-2 font-semibold text-white"
                >
                    Envoyer un message
                </button>
            </div>
