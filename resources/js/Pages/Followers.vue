...
const emit = defineEmits(['closeFollowers']);
const target = ref();
...
const sendFollow = (id) => {
    axios.post('/follow/' + id)
        .then(res => {
            props.followers.filter(u => u.id == id)[0].followedByUser = res.data;
        });
}
        ...
        <div class="w-screen h-screen top-0 backdrop-brightness-50 z-[99] absolute flex justify-center items-center">
            <div class="w-[440px] h-[440px] p-8">
                <div  ref="target" class="bg-[hsl(0,0%,15%)] rounded-xl">
                    <div class="text-white text-center relative flex justify-center border-b gap-8 py-5 border-[hsl(0,0%,25%)]">
                        <div class="px-6 absolute self-start -right-4 ">
                            <svg-icon class="hover:cursor-pointer" size="28" @click="() => emit('closeFollowers')" type="mdi"
                    ...
                    <div class="p-4">
                        <div v-for="(u, index) in followers" :key="index" class="flex justify-between text-white ">
