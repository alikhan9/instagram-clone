...
<template>
  <div
    class="w-full h-full z-[99] backdrop-brightness-50 absolute lg:flex justify-center items-center"
  >
    <div class="lg:w-[440px] lg:h-[440px] h-full w-full lg:p-8">
      <div
        ref="target"
        class="bg-[hsl(0,0%,15%)] flex flex-col lg:block h-full lg:rounded-xl"
      >
        <div
        ...
        <div class="p-4 flex-1 overflow-auto h-full lg:max-h-[380px]">
          ...
          >
            ...
            <button
              v-if="!u.followedByUser"
              @click="sendFollow(u.id)"
              :class="{
                'bg-blue-400 w-36 h-10 rounded-xl': true,
                'bg-gray-600': usePage().props.auth.user.id === u.id,
              }"
              :disabled="usePage().props.auth.user.id === u.id"
            >
              Suivre
            </button>
            <button
              v-else
              @click="sendFollow(u.id)"
              class="bg-red-400 w-36 h-10 rounded-xl"
            >
              Ne plus suivre
            </button>
          </div>
